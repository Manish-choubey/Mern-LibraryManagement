...
const createBooks = async function (req, res) {
  try {
    ...
    const { title, excerpt, userId, ISBN, category, releasedAt, subcategory } =
      data;
    // ------------------------ validation start -------------------------------
    ...
    if (!isValidid) {
      return res
        ...
        .send({
          status: false,
          message:
            "There is no such id in database, Please provide a valid User Id",
        });
    }
    ...
    if (!isRightFormatISBN(ISBN)) {
      return res
        .status(400)
        .send({ status: false, message: "Please provide a valid ISBN" });
    }
    ...
    if (isUniqueISBN) {
      return res
        ...
        .send({
          status: false,
          message: "ISBN already exist, please check your input",
        });
    }
  ...
  try {
    ...
    if (req.query.userId) {
      if (!isValidObjectId(req.query.userId))
        return res
          .status(400)
          .send({ status: false, msg: "Type of UserId must be ObjectId " });
  ...
  try {
    ...
    if (Object.keys(bookId) == 0) {
      return res
        ...
        .send({
          status: false,
          message: "Please Provide a valid bookId in path params",
        });
    }
    ...
    return res
      .status(200)
      .send({ status: true, message: "Books list", data: newData });
  } catch (error) {
    return res.status(500).send({ status: false, message: error.message });
  }
};
