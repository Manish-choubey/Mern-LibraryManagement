...
const createUser = async function (req, res) {
  try {
    ...
    if (
      !(title.trim() == "Mr" || title.trim() == "Miss" || title.trim() == "Mrs")
    ) {
      return res
        .status(400)
        .send({
          status: false,
          message: "Please provide an appropriate title",
        });
    }
...
const login = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "You are successfully logged in", token });
  } catch (error) {
    console.log(error);
    return res.status(500).send({ message: error.message });
  }
